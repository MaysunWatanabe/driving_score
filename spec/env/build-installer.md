<!-- 作成: 2026-08-07 17:33:44 JST | 更新: 2026-08-24 14:12:56 JST -->
...
```json
{
  "required_changes": [
    {
      ...
      "description": "approved facts と既存記述を統合し、Node 18.19.x/.nvmrc 正本および linux 開発機プロファイル（npx cap run / adb フォールバック）を維持した仕様 MD を entrypoint に確定する"
    }
  ],
  "suggested_impacts": [
    {
  ...
  "open_questions": [],
  "rationale_notes": [
    "node_id=env.build.installer の approved facts（Node 18.19.1 / 18.19.x version_match、linux 開発機プロファイル）を真とし、既存 MD 本文と矛盾がなかったため社内配布の Win/Mac 手順・定数・config-npm.bat・batch ラッパは維持した。",
    "センサー周期・モード別ゲート・BLE --rate-ms・12バイト符号化など他ノードの approved facts は本ノードの実行環境範囲外のため本文に取り込まない。",
    "env.app.bootstrap への Node 18.19.x 前提含有は fact 本文が明示している横断事項であり、本ノードでは関連ノードとして参照するに留める。"
  ]
}
```