<!-- 作成: 2026-09-10 17:31:11 JST | 更新: 2026-09-18 17:57:17 JST -->
...
改修要求の出所:
- 日産自動車『運転機能チェックアプリの一次仕様』2026-08-04（全 14 枚）
- メイサンソフト『要求仕様確認』2026-09-17（全 8 枚、先方回答併記）— 特に 4 ページ
...
2026 年度改修要求（全 5 本）のうち、本画面が対象となるのは **③採点スコアのレーダーチャート表示** および付随する **評価コメント切り替え** である。
開発期限は 2026 年 11 月末（12 月から高齢者を招いた実験が開始されるため）。
...
- 「過去の平均」系列は、診断開始前画面 (1-2) の前回結果表示と同じ「履歴平均」概念に対応する。前回の記録が存在しない場合は線を描画しない（[[ui.opening.page]] と整合させる）。

## 評価コメント切り替えタブ（確定）
- **評価コメント切り替えタブは、診断終了後の本画面（5-1 アドバイス表示）にのみ設ける。**
- 目的は「どの評価コメントが効果的か」を利用者アンケートで確認することであり、利用者が複数のコメント表現を切り替えて閲覧できるようにする。
...
## 画面向き（未確定）
- 先方から「縦横変更できるようにしてください」との要求があるが、現行実装は Android で PORTRAIT 固定である。
- 本画面をどちらの向きに対応させるか、横向きでのレーダーチャート＋コメントのレイアウト構成が未確定（open question 参照）。

## データ供給（未確定・凍結中）
- 6 項目の採点データ形式（スコア値・評価メッセージの受け取り方）は先方検討中で「試作中に提示」とされており未確定。
- スコアロジック（`scoreLogicFunction.txt` 系および `scoreLogic.json`）は打ち合わせ後まで**変更凍結**であり、現時点では新仕様の 6 項目値を供給できない。
...
- 参照: [[middleware.score-logicCan]] / [[middleware.sensor-service]] / [[infra.assets-scoreLogicJson]] / [[ui.history.page]] / [[ui.opening.page]] / [[db.capability-score.repository]]

```json
{
  "required_changes": [
    {"node": "ui.comment.page", "entrypoint": "spec/ui/comment-page.md", "description": "2026年度改修要求（☆スコア表示削除・6項目5段階レーダーチャート2系列表示・評価コメント切替タブを本画面のみに設置）を現行仕様と分離した章として追記"},
    {"node": "ui.comment.page", "entrypoint": "spec/ui/comment-page.md", "description": "8-1サービス案表示のタブおよび改修要求②タブ切替と、本画面の評価コメント切替タブが別物である旨の注記を追加"},
    {"node": "ui.comment.page", "entrypoint": "spec/ui/comment-page.md", "description": "☆スコア表示削除により現行唯一の導線（スコア欄タップ）が失われる点を未確定事項として明記"},
    {"node": "ui.comment.page", "entrypoint": "spec/ui/comment-page.md", "description": "縦横変更要求と現行PORTRAIT固定の衝突、6項目データ形式未確定・スコアロジック凍結を未解決事項に追加"}
  ],
  "suggested_impacts": [
  ...
  "fact_candidates": [
    ...
    {
      "type": "display_rule",
      "title": "評価コメントは利用者が切り替えて閲覧できる",
      "statement": "アドバイス表示画面では、どの評価コメントが効果的かをアンケートで確認する目的で、利用者が評価コメントを切り替えて閲覧できる",
      "status": "candidate"
    },
    {
      "type": "display_rule",
    ...
    },
    {
      ...
      "statement": "アドバイス表示画面の評価コメント切り替えタブは、新規画面8-1サービス案表示のサービス案1/サービス案2切り替えタブとは別の機能である",
      "status": "candidate"
    },
    {
      "type": "display_rule",
  ...
  "open_questions": [
    ...
    "評価コメント切り替えの具体仕様が未確定。切り替え可能なコメントの種類数、切替単位（6項目全体一括か項目単位か）、切替結果を保存・記録するか（アンケート集計のため）、タブのラベル表記が資料から読み取れない。確定しないとタブUIとデータ取得契約、保存要否（DB影響）が定義できない。",
    "6項目（筋力・柔軟性・空間把握・危険予測・視力・視野）のスコア値と評価コメントの供給方式が未確定。先方が『試作中に提示します』としており形式が未定で、かつスコアロジック（scoreLogicFunction.txt / scoreLogic.json）は打ち合わせ後まで変更凍結のため、現時点では6項目値を算出・供給できない（Middleware 判断が必要）。表示仕様の実装着手可否に影響する。",
    ...
    "本画面を縦横両対応にするかが未確定。先方は『縦横変更できるようにしてください』と要求しているが、現行は Android で PORTRAIT 固定しており、横向きでのレーダーチャート＋6項目コメントのレイアウト構成も未定（UI + プロダクト判断が必要）。",
  ...
  "rationale_notes": [
    ...
    "承認済み design_decision（node=ui.comment.page）に従い、評価コメント切り替えタブは本画面限定であることを明記し、改修要求②『タブ切り替えの追加』および新規画面8-1のタブと混同しないよう注記を置いた。",
    "現行実装と改修要求が大きく異なるため、md を『現行仕様（実装実態）』と『2026年度改修要求』の2章に分離した。実装凍結中（スコアロジック変更禁止）の状況下で、実装実態の記述を失わずに要求を併記するための構成である。",
    ...
    "既存仕様の補足（joinMessage / getRank / %COUNT の意味 / 履歴画面との役割分担）は真実源に基づく有用な記述であり維持した。",
    "未算出時に 100 が表示されることは仕様上の是非を含む論点だが、承認済みファクトの方針に従い実装実態としてのみ記述し、良否判断は書かない。",
    "『過去の平均』は 1-2 前回結果表示の承認済みファクト（前回＝履歴平均、記録なしは未描画）と同一概念として扱い、集計期間の具体値は未確定として open question に分離した。"
  ]
}
```